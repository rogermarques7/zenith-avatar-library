...
# Sessão 16 — 30/07/2026

**3 avatares fechados, 5 folhas reprovadas, e uma lição de processo que custou
duas gerações.** Fim da sessão: **67 avatares, 28 femininos**.

## O que entrou

| id | descritor | âncora anexa | previsto | medido |
|---|---|---|---|---:|
| `zen_f_b10_d2` | powerlifter feminina / obesidade grau I | nenhuma | 40–42 | **52,6** |
| `zen_f_b10_d1` | mulher no começo da obesidade, não mórbida | nenhuma | 38–42 | **55,1** |
| `zen_f_b02_d3` | corredora de maratona de elite | 18,3 | 16,5–18,5 | **16,9** |

## O erro que atravessou a sessão: âncora que eu nomeava e nunca anexava

Abri a sessão mirando o maior vão `high` da biblioteca feminina e escrevi, em dois
prompts seguidos, *"Âncora `zen_f_b09_d1` (34,1)"* — tratando a âncora como a
referência do meu `sheet_qa`, que é uma das coisas que ela é. A outra, e a que
importa para o resultado, é que **a âncora é uma imagem que o humano anexa no
gerador** (`CHARACTER_BIBLE` §6.2). Eu nunca mandei anexar.

Sem imagem, o substantivo foi sozinho para o atrator obeso do ChatGPT. Os dois
pousos — **+18,2** e **+21,0** sobre a âncora nominal — batem com o passo
sem-âncora que a §2.4 já tinha medido em duas amostras do Gemini (+19,3 e +20,6).
Quatro amostras, dois geradores, mesma regularidade. **Eu andei em cima de uma
lição escrita**, e ela estava no arquivo que o `CLAUDE.md` manda abrir antes de
propor método.

O Rogério fechou a outra metade do mesmo buraco na mesma sessão: *"subir qual?
sempre fala o nome da reference."* Eu vinha repetindo os parâmetros da Meshy e
omitindo os três arquivos que ele precisa pegar no disco.

## O vão `f d1` 34,1 → 42,6 morreu como candidato a folha

Cinco gerações, cinco redações, dois geradores, e o resultado é uma **matriz
completa** (§2.5b):

- **ChatGPT** não tem corpo entre ~34 e ~52 na direção `d1`. Três folhas, todas
  ordinalmente maiores que a que mediu 55,1. É a zona morta da §2.5, uma banda
  acima da que já estava documentada (28–38).
- **Gemini com âncora a 19 pontos do alvo devolve a âncora.** Duas folhas com
  Δ de 0,1 a 0,5 pp em todos os eixos. A segunda trazia *"gere um corpo MUITO
  MAIS PESADO e MUITO MAIS VOLUMOSO"* em parágrafo próprio: **gritar a direção
  não venceu a imagem.**

Declarada saturação e registrado para o híbrido/shape key, que é o 4º degrau da
escada de escalonamento e onde ela de fato termina. A única célula que ninguém
testou é âncora intermediária (~28–31) no Gemini.

## Duas travas de razão nasceram aqui, e as duas pegaram folha ruim

**§1.4b — razão parada = âncora reescalada.** A primeira tentativa do `b03_d2`
tinha todas as larguras subindo os mesmos ~0,5 pp e as razões imóveis
(`cintura/quadril` 0,594 → 0,595; `cintura/ombro` 0,587 → 0,583). Era a mesma
figura 2% maior, e teria virado um duplicado do `b02_d2`. O caso de referência
para o limiar é o `b06h_d3`, que saiu a 0,2 de IMC da âncora e mesmo assim moveu
a razão **0,030** — sete vezes o que eu tinha na mão.

**§1.4c — identidade feminina tem assinatura na folha.** Numa folha do Gemini,
`cintura/quadril` foi 0,594 → **0,651** *com o quadril absoluto caindo* 0,62 pp.
São dois dos três traços que fazem o `zen_f_b09_d3` ler masculino. A folha tinha
o melhor alinhamento do dia (0,14% de variação) e a massa na direção certa:
teria passado por qualquer outra trava. Da última vez esse desvio passou porque
eu estava vigiando resolução; desta vez saiu na medida, antes da Meshy.

## O que funcionou

O `b02_d3` foi o único acerto de previsão do dia, e por um motivo específico: eu
**corrigi o palpite pela medida da folha** (as larguras vieram abaixo da âncora)
em vez de defender o número que tinha derivado do descritor. Ele também confirmou
de novo o mecanismo da §3.5 — âncora de 18,3 segurou o peso (pousou em 16,9) e o
descritor `d3` moveu o tônus (**relevo abdominal +35,8%**). E abriu o pé da linha
`d3`, que não tinha nada abaixo de 21,0.

Três folhas mostraram a **assinatura de vazamento** do `sheet_qa`, incluindo duas
do ChatGPT — mais evidência de que a assinatura é do método e não do gerador
(§1.1). Nas três o remédio foi remedir em outro limiar: `BG_TOL=48` numa âncora
com fundo em gradiente, `BG_TOL=12` na folha do `b02_d3`, que na leitura padrão
acusava 6,67% de variação de altura e na remedição deu **0,34%**.

## Parado a pedido dele

O `f_b03_d2` fica para o fim. A categoria "bailarina clássica de companhia
profissional" acerta a forma (`cintura/ombro` 0,587 → 0,549, movimento real) mas
carrega um corpo **mais leve** que a âncora de 18,3, e o `b03` precisa ser mais
pesado que o `b02`. Ao retomar: mesma categoria, âncora `zen_f_b04_d2` (22,2).

---

# Sessao 17 - 31/07/2026, madrugada

A sessao que fechou a `f d1` e a `f d3`. Sete avatares, tres folhas reprovadas,
74 avatares no total - e uma licao que reorganiza toda a mira: **o sinal do passo
domina o tamanho dele**.

## O que entrou

| id | descritor | ancora | previsto | medido | passo |
|---|---|---:|---|---:|---:|
| `b05_d1` | sedentaria no comeco do sobrepeso | 24,1 | 28-31 | 44,4 | +20,3 |
| `b11_d2` | rugby feminino, primeira linha | 34,4 | 42-46 | 59,7 | +25,3 |
| `b10_d3` | open bodybuilding / Ms. Olympia | 30,1 | 36-42 | 45,1 | +15,0 |
| `b04_d3` | atleta de Figure de competicao | 22,3 | ~25 | 29,9 | +7,6 |
| `b01_d2` | corredora de rua franzina | 18,3 | 16-17 | 17,2 | -1,1 |
| `b02_d1` | franzina sedentaria | 19,0 | 17,5-18,5 | 16,4 | -2,6 |
| `b03_d3` | atleta de salto em altura | 21,0 | ~19 | 16,1 | -4,9 |

Todos 8/8 no `process.py`. Nenhuma trava sumiu.

## A descoberta: subindo estoura, descendo e fino

As quatro primeiras foram para cima e as quatro estouraram a faixa declarada. O
`b05_d1` mirava o vao 24,1-31,9 e pousou em 44,4, do outro lado dele. O `b11_d2`
mirava o meio do vao 34,4-52,6 e pousou em 59,7, acima do topo da linha inteira.

A quarta, o Figure, foi a mais moderada que eu consegui escrever - uma divisao de
competicao que fica literalmente entre duas ja medidas (bikini 22,3 e wellness
27,7) - e ainda assim deu **+7,6**, maior que qualquer vao que restava fechar.

Ai vieram as descidas: -1,1, -2,6, -4,9. E o `b01_d2` foi **a unica previsao da
sessao a cair dentro da faixa declarada**.

A causa e empilhamento de lever, e e a mesma licao da §2.2c aplicada a direcao:
ancora-com-clausula-de-direcao e um lever, categoria mais pesada que a da ancora
e outro, e os dois apontando para o mesmo lado somam. Nas descidas eu usei
categoria moderada; nas subidas usei categoria extrema E direcao.

## O que a negacao nao faz

Tres clausulas de negacao explicitas falharam na mesma sessao:

- *"Ela NAO E OBESA: sem dobras, sem avental"* -> IMC 44,4, obesidade grau III.
- *"ELA E UMA MULHER... NAO tem peitoral masculino no lugar do busto"* -> o
  `b10_d3` saiu fisiculturista masculino caricato.
- *"os OMBROS DELA SAO ESTREITOS"* -> o ombro alargou, `ombro/quadril` foi de
  1,030 para 1,103.

E a mesma clausula de quadril funcionou nos dois acertos (`b01_d2` e `b02_d1`).
A diferenca e se havia atrator puxando contra. Negacao nao e lever quando ha.

## A regua que nasceu: `ombro/quadril` na folha 2D

Depois de reprovar duas folhas do `b03_d3` no olho e no palpite, parei e medi as
dez folhas `d3` que ja viraram avatar, comparando com o parecer de anatomia de
cada corpo. As seis que geraram corpo feminino ficam entre **0,940 e 1,050**; a
que gerou o `b09_d3` masculino da **1,212**. As duas folhas reprovadas davam
1,103 e 1,104; a terceira, aprovada, deu 1,059, e o `b02_d1` deu 1,029.

Duas folhas mediram 0,462 e 0,148 - a regua do ombro quebrando, porque o
`sheet_qa` mede numa `at_frac` fixa de 0,194 que cai fora do deltoide quando a
proporcao muda. Mesma familia da pegadinha do `thigh`.

Tambem descobri que a §1.4c (cintura/quadril subindo com quadril caindo) da
**falso positivo em passo descendente**: quando o corpo inteiro encolhe, o
quadril cai em absoluto por construcao. As tres descidas aprovadas disparam a
assinatura e vieram femininas.

## O `b10_d3`, o segundo asset que le masculino

IMC 45,1, 8/8, e uma caricatura: trapezio engolindo o pescoco, delcoides e bracos
fora de escala humana, mandibula masculina, busto virado peitoral. A trava de
identidade feminina estava no prompt, literal, e nao segurou.

Regra 5b: fica. Mas agora a `f d3` tem os dois corpos do topo lendo masculino -
32,4 e 45,1 - e como o campo `approved` continua morto, os dois sao servidos.

## O `sheet_qa` vazou em 5 das 10 folhas

Leu figura a partir de `y=0`, base 45 px abaixo do pe, altura 8,54% divergente
onde o `crop.py` lia 0,43%. Em todos os casos o remedio foi o mesmo: rodar o
`crop.py` num id descartavel (`zen_f_b99_*`) com `--force`, medir, e apagar a
folha b99. O `crop.py` e o detector do caminho do produto e ele nunca errou.

## O Gemini nao chegou a ser testado

Duas folhas do Gemini chegaram naquela noite e as duas falharam **na entrega**:
uma veio byte a byte identica a ancora anexada (SHA-256 igual ao
`zen_f_b05_d3_sheet.png` - o download trouxe o anexo de volta), a outra veio com
o terco direito cinza vazio, faixa marrom no rodape, fundo de ruido 187 e os
bracos dissolvidos.

Nenhuma das duas e evidencia sobre o Gemini como gerador. Confundir isso com
atrator desperdicaria a unica alternativa que existe quando o ChatGPT satura.

## As duas zonas mortas

A `f d1` 34,1-42,6 ja estava declarada saturada na sessao 16. Nesta sessao a
`f d2` 34,4-52,6 se juntou a ela: duas categorias pesadas distintas no ChatGPT -
rugby de primeira linha e arremesso de peso - pousaram no mesmo ponto, ~58-60.
E a mesma banda de peso. O atrator feminino pesado do ChatGPT mora ali, e a faixa
35-50 nao existe nele.

E caso de shape key no hibrido. A celula ainda nao testada nas duas e ancora
intermediaria (~28-31) no Gemini.

## Onde a sessao parou

`f d1` e `f d3` completas. Faltam 3 slots, todos na `f d2`: o `b03` parado a
pedido dele desde a sessao 16, e o `b07` e o `b08` na zona morta.

Nasceu o `qa/probe/contagem_slots_f.py`, que deriva a contagem do `library.json`
em vez de contar a mao - a `d3` comeca em `b02` e termina em `b10`, e um range
`b01..b09` inventa um slot e esconde o `b10`, o que ja aconteceu duas vezes.

E ele decidiu o proximo passo: fechar os 3 slots e **reabrir a frente do short**,
agora com duas pecas no feminino (faixa + short). A frente estava parada desde
28/07 esperando exatamente a condicao que ele pos: as duas bibliotecas existirem.

---

# Sessao 18 - 31/07/2026, manha - o credito acabou

Dois avatares fechados, os dois 8/8, os dois DESCENDO. E a sessao terminou por
falta de credito na Meshy, nao por falta de metodo.

## zen_f_b03_d2 - IMC 19,8, o slot que estava parado desde a sessao 16

O `b03` da `f d2` estava parado a pedido do Rogerio, com a receita ja escrita:
categoria "bailarina classica de companhia profissional", ancora `zen_f_b04_d2`
(22,2). Alvo ~20, entre o 18,3 e o 22,2.

Um lever so: a categoria. Sem cláusula de direcao empilhada em cima, porque a
§2.4c da sessao 17 mediu que empilhar dois levers na mesma direcao atravessa o
vao inteiro.

A folha leu sa no `sheet_qa`: topo em y=45 (sem assinatura de vazamento),
variacao de altura 0,76%, `ombro/quadril` 1,039 (dentro de 0,940-1,050). As
razoes se moveram - `cintura/ombro` 0,611 -> 0,565 - entao nao era a ancora
reescalada. O ombro segurou (-0,24 pp) enquanto quadril (-1,65), cintura (-1,21)
e coxa (-0,99) recuaram: a linha da bailarina, estreita embaixo. A sonda de tonus
deu abdomen -12,2% e peitoral -15,6%, que e o que a `d2` pede.

Mediu **19,8**. Alvo era ~20. Passo -2,4.

## O credito acabou, e isso virou uma decisao de produto

No meio da sessao ele avisou: sobrou credito para UM avatar. Faltavam dois slots
de grade, `f d2 b07` e `b08`, os dois na zona morta.

Eu abri quatro opcoes e ele cobrou, com razao: "vc disse que faltava apenas 2,
entao pq me deu 4 opcoes?". Eu tinha misturado duas contagens sem avisar - SLOT
DE GRADE (contagem fechada, 2) e VAO DE COBERTURA (insercao pela regra 5b, sem
numero fixo). Sao coisas diferentes e eu apresentei como se fossem a mesma lista.

O que me fez abrir alem dos 2 foi um achado no `build_index.py`: o vao que o
`b07`/`b08` fechariam e `f d2` 34,4 -> 52,6, cujo **meio e 43,5**, fora do
`USER_BMI_RANGE = (17, 40)` da linha 188. O proprio indice classifica esse vao
como **`low`** - e ele nem aparecia na lista impressa, que e truncada em
`gaps[:8]`. Ou seja: fechar a grade ali seria gastar o ultimo credito num buraco
que o codigo do projeto ja declara irrelevante para usuario real, E na zona morta
onde duas categorias distintas pousaram em ~58-60.

Reapresentei com uma recomendacao unica e ele escolheu: insercao no vao `high`
`f d1` 24,1 -> 31,9.

**A licao nao e sobre creditos.** Quando o recurso e escasso, "fechar a contagem"
e um atrator tao forte quanto os do gerador. O `CLAUDE.md` ja dizia "a meta e
COBERTURA, nao contagem", e a evidencia que desempatou estava dentro do proprio
`build_index.py`, nao numa opiniao minha.

## zen_f_b04i_d1 - IMC 26,5, o lever-ancora usado isolado pela primeira vez

Vao de 7,8 pontos, alvo ~28, passo necessario -3,9. O `CHARACTER_BIBLE` ja dizia
que vao menor que ~6,3 nao se fecha trocando descritor: o lever passa a ser a
ancora. Nunca tinha sido usado sozinho.

O descritor manteve a categoria da ancora (`zen_f_b07_d1`, 31,9, sedentaria sem
tonus) e mandou so "o MESMO tipo de corpo, porem mais leve", com uma lista do que
MUDA (barriga menor, quadril menos largo, coxas menos grossas, bracos menos
cheios) e outra do que NAO MUDA (ausencia total de tonus, contornos redondos,
cintura sem afunilamento, ombros caidos).

A folha veio com o melhor alinhamento da sessao: 0 px de espalhamento no topo,
2 px nos pes, 0,22% de variacao de altura.

**Um susto que quase custou o credito, e a licao dele.** Olhando a folha inteira
em miniatura, li a vista de perfil como mais magra que a frente e as costas - o
que, se fosse verdade, faria a Meshy fundir tres corpos diferentes. As medidas
nao acusavam (razao perfil/frente do quadril 0,613, dentro do padrao da serie).
Recortei o perfil da folha nova e o da ancora, colei lado a lado ampliados, e
estava errado: e a mesma familia de corpo, so mais leve. Fiz o mesmo com a
frente. As duas coerentes.

**Miniatura nao e leitura de anatomia.** O `Read` de uma folha de 1536x1024
devolve algo perto de 384x256, e nessa escala a diferenca entre "mais leve" e
"outro corpo" nao existe. Quando a decisao for cara, recortar e ampliar - custa
segundos e e a diferenca entre reprovar uma folha boa e aprovar uma ruim.

Mediu **26,5**. Eu tinha previsto 27-29. **Estourou para baixo**, e o passo foi
**-5,4** - a maior descida ja medida no projeto, contra o recorde anterior de
-4,9.

## O que a 18 acrescenta a §2.4c

A sessao 17 concluiu "subida estoura, descida acerta". A 18 mede o contorno
disso: **descida tambem erra**. A diferenca e a ordem de grandeza - a descida
errou por 1,5 ponto (previ 27-29, mediu 26,5), enquanto as subidas da 17 erraram
por 7,6 a 25,3. O envelope de descida passa a ser **-5,4**, nao -4,9.

Nenhuma folha foi reprovada nesta sessao. As duas passaram em todas as travas de
primeira, o que nao acontecia desde a sessao 12.

## Onde a sessao parou

76 avatares - 39 masculinos e 37 femininos. `f d1` e `f d3` completas, `f d2`
com 2 slots (`b07`, `b08`) que ficam esperando credito - e que, quando o credito
voltar, provavelmente NAO devem ser o destino dele.

Sem credito, a unica frente que anda e a do SHORT, que mexe so em malha ja
produzida: 11 masculinos na fila de correcao e os 37 femininos sem nenhuma peca
pintada, precisando de duas (faixa + short).
